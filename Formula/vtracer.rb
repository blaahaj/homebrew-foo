class Vtracer < Formula
  ...
  url "https://github.com/visioncortex/vtracer.git", tag: "0.6.5"
  ...
  test do
    ...
    Dir.mktmpdir do |tmpdir|
      ...
      png_content = base64.gsub(/\s+/, "").unpack1("m0")
      File.write("#{tmpdir}/circle.png", png_content)

      system "#{bin}/vtracer -i circle.png -o circle.svg --mode polygon", chdir: tmpdir
      raise "vtracer command failed (#{$CHILD_STATUS.inspect})" unless $CHILD_STATUS.success?

      svg_content = File.read("#{tmpdir}/circle.svg")
      raise "vtracer output file doesn't contain SVG" unless svg_content.strip.end_with?("</svg>")
    end
  end
end
